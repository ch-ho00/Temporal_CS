...
class IntervalLoss(nn.Module):
    ...
    def forward(self, normed_y, label, interval_pred, alpha=0.1, soften = 80., lambda_in = 8., weight=0.1):
        ...
        if n > 0:
            y_U_cap = y_pred_U[label == 0] > normed_ypos.reshape(-1)
            y_L_cap = y_pred_L[label == 0] < normed_ypos.reshape(-1)
            k_hard = y_U_cap*y_L_cap
            PICP = torch.sum(k_hard)/n
            
            MPIW_cap = torch.sum(k_hard * (y_pred_U[label == 0] - y_pred_L[label == 0])) / (torch.sum(k_hard) + 0.001)
        ...
        if n > 0:
            y_U_neg = y_pred_U[label == 1] > normed_yneg.reshape(-1)
            y_L_neg = y_pred_L[label == 1] < normed_yneg.reshape(-1)
            ...
            neg_loss = torch.sum((y_U_neg*y_L_neg).int()) # + lambda_in * (n**0.5) * (max(0,(1-alpha)-PICP_neg)**2)

            pred_label[label ==1] = k_hard_neg.long()

        print(pos_loss, neg_loss)
        return (pos_loss + neg_loss) * weight, torch.sum(k_hard) + torch.sum(k_hard_neg), pred_label
